package {
	...
	public class Maps extends Sprite {
		...
		private function mapMousedown(evt:MouseEvent):void {
			var endX=Math.floor((mouseX-map.x)/ wh);//将鼠标点击位置转化为节点索引值
			var endY=Math.floor((mouseY-map.y)/ wh);//将鼠标点击位置转化为节点索引值
			var endPoint=mapArr[endY][endX];//从地图中取出鼠标点击的节点作为寻路终点
			//如果目的地是可通过的则开始寻路
			if (endPoint.go == 0) {
				//每次寻路开始前将上次的路径清空
				if (roadList) {
